...
!if (eex .gt. 0.0) print*, 'absorption', aia(1), aia(mdfs), 1.0 - eex
	   ...
	   if (k .ne. i) then  
	     ...
	     if (k .lt. i) then 
	      ! k is above i
	       if (i .eq. mdfs) then
	         ...
	         e_exch = fid_source(k,j) * (tbar(k) / pts - 1.0) * aia2(i,j) + fiu(i,j) * (pts / tbar(k) - 1.0) * aia2(k,j)
	       else
	         ! first term k is origin,   second term k is destination
	         e_exch = fid_source(k,j) * (tbar(k) / tbar(i) - 1.0) * aia2(i,j) + fiu_source(i,j) * (tbar(i) / tbar(k) - 1.0) * aia2(k,j)
	       endif	     
	     ...
	     elseif (k .gt. i) then
	       ...
	       if (k .eq. mdfs) then
	         ...
	         e_exch = fiu(k,j) * (pts / tbar(i) - 1.0) * aia2(i,j) + fid_source(i,j) * (tbar(i) / pts - 1.0) * aia2(k,j)
	       else
	         ! first term k is origon,      second term k is destination
	         e_exch = fiu_source(k,j) * (tbar(k) / tbar(i) - 1.0) * aia2(i,j) + fid_source(i,j) * (tbar(i) / tbar(k) - 1.0) * aia2(k,j)
	       endif
...
! this is my output but it does not work
